      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            fontFamily: 'Outfit, sans-serif', fontSize: '0.875rem', cursor: 'pointer',
          }}
        >
          Enviar otro mensaje
        </button>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div className="form-group">
        ...
        <input id="name" type="text" {...register('name')} className={`form-input ${errors.name ? 'form-input--error' : ''}`} placeholder="Tu nombre completo" autoComplete="name" />
        {errors.name && <span className="form-error" role="alert">{errors.name.message}</span>}
      </div>

      <div className="form-group">
        <label htmlFor="email" className="form-label">Email *</label>
        <input id="email" type="email" {...register('email')} className={`form-input ${errors.email ? 'form-input--error' : ''}`} placeholder="tu@empresa.com" autoComplete="email" />
      ...
      <div className="form-group">
        ...
        <input id="company" type="text" {...register('company')} className="form-input" placeholder="Nombre de tu empresa" autoComplete="organization" />
      </div>

      <div className="form-group">
        <label htmlFor="service" className="form-label">¿Qué necesitas? *</label>
        <select id="service" {...register('service')} className={`form-input form-select ${errors.service ? 'form-input--error' : ''}`}>
      ...
      <div className="form-group">
        ...
        <textarea id="message" {...register('message')} className={`form-input form-textarea ${errors.message ? 'form-input--error' : ''}`} placeholder="Describe qué quieres construir, qué problema tienes o qué resultado buscas..." rows={5} />
      ...
      <button type="submit" disabled={isSubmitting || status === 'loading'} className="form-submit">
        {status === 'loading' ? 'Enviando...' : 'Enviar mensaje →'}
      </button>

      <style>{`
        .form-group { display: flex; flex-direction: column; gap: 0.5rem; }
        .form-label { font-family: 'Outfit', sans-serif; font-size: 0.875rem; font-weight: 600; color: #A8A29E; }
        .optional { font-weight: 400; color: #57534E; }
        .form-input { width: 100%; padding: 0.75rem 1rem; background: #0E0D0C; border: 1px solid #292524; border-radius: 8px; color: #F5F4F2; font-family: 'Source Sans 3', sans-serif; font-size: 0.95rem; line-height: 1.5; transition: border-color 0.2s ease, box-shadow 0.2s ease; outline: none; -webkit-appearance: none; }
        .form-input::placeholder { color: #57534E; }
        .form-input:focus { border-color: #EA580C; box-shadow: 0 0 0 3px rgba(234, 88, 12, 0.15); }
        ...
        .form-error { font-family: 'Outfit', sans-serif; font-size: 0.8rem; color: #EF4444; }
        .form-error-global { padding: 0.875rem 1rem; background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; font-family: 'Outfit', sans-serif; font-size: 0.875rem; color: #EF4444; }
        .form-submit { padding: 0.875rem 2rem; background: linear-gradient(135deg, #9A3412 0%, #EA580C 40%, #D97706 70%, #FCD34D 100%); color: #050505; font-family: 'Outfit', sans-serif; font-size: 1rem; font-weight: 700; border: none; border-radius: 8px; cursor: pointer; transition: opacity 0.2s ease, box-shadow 0.2s ease; margin-top: 0.5rem; }
        .form-submit:hover:not(:disabled) { opacity: 0.9; box-shadow: 0 0 30px rgba(234, 88, 12, 0.35); }
